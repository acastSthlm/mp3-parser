...
(function (globalObject, createModule) {
    ...
    var id3v2TagFrameNames = {
        ...
        //  The frame contains subframes, typically TIT2, and possibly additional tags
        //
        ...
        readFrameContentChap = function (view, offset, length) {
          ...
          // Locate the the null terminator seperating MIME type and picture type
            offsetTrm = lib.locateStrTrm.ucs(view, 0, length - 1);
          if (offsetTrm === -1) {
            return content; // Not found!
          }

          // Read start time
          content.startTime = view.getUint32(offset + offsetTrm + 1);

          // Read end time
          content.endTime = view.getUint32(offset + offsetTrm + 5);

          // Read start offset
          content.startOffset = view.getUint32(offset + offsetTrm + 9);

          // Read end offset
          content.endOffset = view.getUint32(offset + offsetTrm + 13);

          var offsetSubFrames = offsetTrm + 17;
          content.frames = [];
          while(offsetSubFrames < length) {
            var subFrame = mp3Id3v2Parser.readId3v2TagFrame(view, offset + offsetSubFrames);
